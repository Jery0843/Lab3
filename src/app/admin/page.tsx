  ...
  if (loading || !isAuthenticated) {
    return (
      <div className="fixed inset-0 bg-black">
        {/* Completely blank screen - no loading indicators */}
      </div>
    );
  }
...
function StatsModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-black/90 light:bg-white/95 border border-white/20 light:border-gray-300 rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className={`text-2xl font-bold ${type === 'HTB' ? 'text-cyber-green' : 'text-cyber-purple'}`}>
            Update {type} Statistics
          </h2>
          <button onClick={onClose} className="text-white light:text-gray-800 hover:text-gray-300 light:hover:text-gray-600">
            <FaTimes />
          </button>
        </div>
        ...
        <div className="flex justify-end mt-6">
          <button
            onClick={onClose}
            className="px-6 py-2 bg-gray-600 text-white light:bg-gray-500 light:text-white rounded hover:bg-gray-700 light:hover:bg-gray-600 transition-colors"
          >
            Close
          </button>
        </div>
